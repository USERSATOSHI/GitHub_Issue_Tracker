...
class PopupManager {
	...
	async loadSettings() {
		try {
			...
			if (settings.projectOwner) {
				document.getElementById('project-owner').value = settings.projectOwner;
			}
			if (settings.projectNumber) {
				document.getElementById('project-number').value = settings.projectNumber;
			}

			// Load Project ID if saved
			if (settings.projectId) {
				document.getElementById('project-id').value = settings.projectId;
			}

			// Update emojis if saved
			if (settings.todoEmoji) {
				document.getElementById('todo-emoji').value = settings.todoEmoji;
			}
			if (settings.progressEmoji) {
				document.getElementById('progress-emoji').value = settings.progressEmoji;
			}
			if (settings.doneEmoji) {
				document.getElementById('done-emoji').value = settings.doneEmoji;
			}

			// Load allowed repos and apply-to-all
			if (settings.allowedRepos) {
				document.getElementById('allowed-repos').value = settings.allowedRepos.join('\n');
			}
			document.getElementById('apply-to-all').checked = !!settings.applyToAll;

			// Load column IDs if saved
			if (settings.todoColumnId) {
				document.getElementById('todo-column-id').value = settings.todoColumnId;
			}
			if (settings.inProgressColumnId) {
				document.getElementById('inprogress-column-id').value = settings.inProgressColumnId;
			}
			if (settings.doneColumnId) {
				document.getElementById('done-column-id').value = settings.doneColumnId;
			}

			// Load Status Field ID if saved
			if (settings.statusFieldId) {
				document.getElementById('status-field-id').value = settings.statusFieldId;
			}
	...
	async saveSettings(showFeedback = true) {
		const settings = {
			...
			inProgressColumnId: document.getElementById('inprogress-column-id').value,
	...
	updateStatus(settings) {
		...
		const hasRequiredSettings =
			settings.githubToken && settings.projectOwner && settings.projectNumber;
